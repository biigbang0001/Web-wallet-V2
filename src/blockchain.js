import { NODE_CONFIG, HD_CONFIG, TRANSACTION_CONFIG, VALIDATION_PATTERNS, NITO_NETWORK, getTranslation, sleep } from './config.js';
...
export async function getTxDetailCached(txid) {
  ...
  const tx = await handleError500WithRetry(async () => {
    return await rpcClient.call('getrawtransaction', [txid, true]);
  });
...
export function extractOpReturnData(scriptHex) {
  ...
  if (!VALIDATION_PATTERNS.SCRIPT_HEX.test(scriptHex)) return null;
...
export async function getTransactionSenderAddress(txid) {
  try {
    ...
    const prevTx = await handleError500WithRetry(async () => {
      return await rpcClient.call('getrawtransaction', [prevTxid, true]);
    });
  ...
  async executeRequest(method, params) {
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        if (attempt === maxRetries) {
          console.error(`[BLOCKCHAIN] ${getTranslation('errors.broadcast_failed', 'Broadcast failed')} ${maxRetries} attempts:`, method, params, error);
...
export class HDUTXOScanner {
  ...
  async scanHDUTXOsWithDescriptors(hdWallet, addressFamily) {
    try {
      ...
      console.warn(`[BLOCKCHAIN] Descriptor scan failed for ${addressFamily}, falling back to legacy scan:`, error);
  ...
  async scanHDUTXOsLegacy(hdWallet, family) {
    ...
    for (let chunk = 0; chunk < HD_CONFIG.SCAN_MAX_CHUNKS; chunk++) {
      ...
      try {
        ...
        console.error(`[BLOCKCHAIN] Legacy HD scan failed for chunk ${chunk}:`, error);
        break;
      }
    }
  ...
  deriveAddressChunk(hdWallet, family, start, count) {
    ...
    try {
      ...
      for (let chain = 0; chain <= 1; chain++) {
        ...
        for (let i = start; i < start + count; i++) {
          ...
          if (!window.bitcoin || !window.ECPair) {
            console.warn(`[BLOCKCHAIN] ${getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin library unavailable')}`);
            continue;
          }
      ...
      console.error(`[BLOCKCHAIN] ${getTranslation('wallet.failed_to_derive_addresses', 'Failed to derive addresses: {{error}}', { error: error.message })}`);
    }
  ...
  deriveAddressForFamily(keyPair, family) {
    ...
    try {
      ...
      console.error(`[BLOCKCHAIN] ${getTranslation('wallet.failed_to_create_taproot', 'Failed to create Taproot address')}`, error);
      return null;
    }
  }
  ...
  async scanAllFamilies(hdWallet) {
    ...
    for (const family of families) {
      try {
        ...
        console.warn(`[BLOCKCHAIN] Failed to scan ${family} UTXOs:`, error);
      }
    }
  ...
  async scanSpecificFamily(hdWallet, family) {
    try {
      ...
      console.error(`[BLOCKCHAIN] Failed to scan ${family} UTXOs:`, error);
      return [];
    }
  }
}
...
export function clearBlockchainCaches() {
  ...
  if (typeof window.clearMessagingCaches === 'function') {
