...
import { NODE_CONFIG, HD_CONFIG, TRANSACTION_CONFIG, VALIDATION_PATTERNS } from './config.js';
...
async function handleError500(method, params, attempt, maxRetries, error) {
  ...
  // Si scan déjà en cours, l'aborter d'abord
  if (error.message && error.message.includes('Scan already in progress') && method === 'scantxoutset') {
    try {
      await rpcClient.call('scantxoutset', ['abort']);
      await sleep(1000); // Attendre que l'abort soit effectif
    } catch (abortError) {
...
export class HDUTXOScanner {
  ...
  deriveAddressForFamily(keyPair, family) {
    ...
    const network = window.NITO_NETWORK;
...
export async function utxos(address, isHD = false, hdWallet = null) {
  try {
    ...
    if (UTXO_CACHE.has(cacheKey)) {
      const cached = UTXO_CACHE.get(cacheKey);
      if (Date.now() - cached.timestamp < 30000) {
        return cached.data;
      }
      UTXO_CACHE.delete(cacheKey);
    }
...
export async function balance(address, isHD = false, hdWallet = null) {
  try {
    ...
    if (BALANCE_CACHE.has(cacheKey)) {
      const cached = BALANCE_CACHE.get(cacheKey);
      if (Date.now() - cached.timestamp < 30000) {
        return cached.data;
      }
      BALANCE_CACHE.delete(cacheKey);
    }
