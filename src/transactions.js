...
export async function consolidateUtxos() {
  ...
  try {
    ...
    const feeManager = new FeeManager();
    const realFeeRate = await feeManager.getRealFeeRate();
    const vbytesPerBatch = feeManager.estimateVBytes(inputType, MAX_UTXOS_PER_BATCH, 1);
    const feesPerBatch = feeManager.calculateFeeForVsize(vbytesPerBatch, realFeeRate) / 1e8;
    const minBatchValue = feesPerBatch * 1.1;

    console.log(`[CONSOLIDATION] Min batch value required: ${minBatchValue.toFixed(8)} NITO`);

    const batches = [];
    const used = new Set();
    const sortedByValue = [...allUtxos].sort((a, b) => b.amount - a.amount);
    const sortedBySize = [...allUtxos].sort((a, b) => a.amount - b.amount);

    while (used.size < allUtxos.length) {
      const batch = [];
      let batchValue = 0;

      for (const utxo of sortedByValue) {
        if (used.has(utxo.txid + ':' + utxo.vout)) continue;

        batch.push(utxo);
        batchValue += utxo.amount;
        used.add(utxo.txid + ':' + utxo.vout);

        if (batchValue >= minBatchValue) break;
        if (batch.length >= MAX_UTXOS_PER_BATCH) break;
      }

      for (const utxo of sortedBySize) {
        if (used.has(utxo.txid + ':' + utxo.vout)) continue;
        if (batch.length >= MAX_UTXOS_PER_BATCH) break;
      ...
      if (batch.length > 0) {
        console.log(`[CONSOLIDATION] Batch ${batches.length + 1}: ${batch.length} UTXOs, value: ${batchValue.toFixed(8)} NITO`);
        batches.push(batch);
      }
    }

    console.log(`[CONSOLIDATION] Created ${batches.length} balanced batches from ${allUtxos.length} UTXOs`);
