  ...
  try {
    ...
    if (largeUtxos.length > 0 && numBatches > 1) {
      ...
      for (let i = 1; i < numBatches; i++) {
        ...
        console.log(`[CONSOLIDATION] Fee UTXO ${i}: ${(feeOutputValue / 1e8).toFixed(8)} NITO for batch ${i + 1} (${batchSize} UTXOs)`);
      }
      ...
      if (changeValue > 1000) {
        ...
        console.log(`[CONSOLIDATION] Change output: ${(changeValue / 1e8).toFixed(8)} NITO`);
      }
      ...
      console.log(`[CONSOLIDATION] Strategy: Batches 2-3 will use 0-conf, batch 4+ will wait for confirmation`);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      // Wait for confirmation before batch 4 (index 3)
      if (i === 3 && batches.length > 3 && splitTxid) {
        console.log('[CONSOLIDATION] Batch 4 reached - waiting for split TX confirmation...');
        if (window.showSimpleProgressBar) {
          window.showSimpleProgressBar(0);
        }
        ...
        if (!confirmed) {
          console.warn('[CONSOLIDATION] Split TX not confirmed after 10 minutes, stopping');
          throw new Error('Split transaction not confirmed in time');
        }
        
        console.log('[CONSOLIDATION] Split TX confirmed - continuing with remaining batches');
      }
      ...
      if (i > 0 && feeUtxos.length > 0 && (i - 1) < feeUtxos.length) {
        ...
        const feeStatus = i < 3 ? 'mempool (0-conf)' : 'confirmed TX';
