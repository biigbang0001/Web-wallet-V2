...
export async function calculateMaxSendableAmount() {
  ...
  try {
    ...
    const usableUtxos = filtered.usableUtxos;
    ...
    const feeInfoNoChange = await feeManager.getGuaranteedFee(inputType, usableUtxos.length, 1);
    const maxNoChange = Math.max(0, totalUsable - feeInfoNoChange.feeNito);
    
    const feeInfoWithChange = await feeManager.getGuaranteedFee(inputType, usableUtxos.length, 2);
    const maxWithChange = Math.max(0, totalUsable - feeInfoWithChange.feeNito);
    
    const bestMax = Math.max(maxNoChange, maxWithChange);
    
    const safetyMargin = Math.max(bestMax * 0.0001, 0.00000001);
    const finalMax = Math.max(0, bestMax - safetyMargin);
    
    console.log(`[MAX] Total usable: ${totalUsable.toFixed(8)} NITO, UTXOs: ${usableUtxos.length}, Fee (no change): ${feeInfoNoChange.feeNito.toFixed(8)}, Fee (with change): ${feeInfoWithChange.feeNito.toFixed(8)}, Max sendable: ${finalMax.toFixed(8)} NITO`);
  ...
  async buildAndSignTransaction(to, target, selectedUtxos, isConsolidation, sourceAddressType) {
    ...
    if (!isConsolidation) {
      ...
      if (workingUtxos.length === 0) {
        throw new Error('Aucun UTXO suffisamment grand disponible pour cette transaction');
      }
    }
