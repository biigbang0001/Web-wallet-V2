...
function filterUtxosByContext(utxos, isConsolidation = false) {
...
export class FeeManager {
  ...
  async getRealFeeRate() {
    ...
    try {
      ...
      const safeMinRate = this.minFeeRate * 1.5;
  ...
  calculateFeeForVsize(vbytes, feeRate) {
    ...
    return Math.max(feeSats, minFeeSats);
  }
...
export async function calculateMaxSendableAmount() {
  ...
  try {
    ...
    const filtered = filterUtxosByContext(workingUtxos, false);
    
    if (!filtered.usableUtxos.length) {
      return 0;
    }

    const usableUtxos = filtered.usableUtxos.sort((a, b) => b.amount - a.amount);
    const totalUsable = usableUtxos.reduce((sum, u) => sum + u.amount, 0);
    ...
    for (const utxo of usableUtxos) {
      ...
      const vbytesNoChange = feeManager.estimateVBytes(inputType, selectedUtxos.length, 1);
      const feesNoChange = feeManager.calculateFeeForVsize(vbytesNoChange, realFeeRate) / 1e8;
      const maxNoChange = Math.max(0, total - feesNoChange);
      
      const vbytesWithChange = feeManager.estimateVBytes(inputType, selectedUtxos.length, 2);
      const feesWithChange = feeManager.calculateFeeForVsize(vbytesWithChange, realFeeRate) / 1e8;
      const maxWithChange = Math.max(0, total - feesWithChange);
      
      const currentMax = Math.max(maxNoChange, maxWithChange);
      
      if (currentMax > bestMax) {
        bestMax = currentMax;
      }
      
      if (total >= totalUsable * 0.98) {
        break;
      }
    }
    
    const safetyMargin = Math.max(bestMax * 0.0001, 0.00000001);
    const finalMax = Math.max(0, bestMax - safetyMargin);
    
    console.log(`[MAX] Total usable: ${totalUsable.toFixed(8)} NITO, UTXOs: ${usableUtxos.length}, Max sendable: ${finalMax.toFixed(8)} NITO`);
...
export class SimpleTransactionBuilder {
  ...
  async signTxWithPSBT(to, amt, isConsolidation = false) {
    ...
    const walletInfo = await getWalletInfo();
    if (!walletInfo.isReady) {
      const errorMsg = window.getTranslation('errors.import_first', 'Import a wallet first.');
    ...
    if (!window.utxos) {
      const errorMsg = window.getTranslation('errors.transaction_functions_unavailable', 'Transaction functions unavailable');
      throw new Error(errorMsg);
    }
    ...
    if (!rawUtxos.length) {
      const errorMsg = window.getTranslation('transactions.no_utxos_for_consolidation', 'No UTXOs available');
      throw new Error(errorMsg);
    }
    ...
    if (!matureUtxos.length) {
      const errorMsg = window.getTranslation('transactions.no_suitable_utxos', 'No suitable mature UTXOs available');
      throw new Error(errorMsg);
    }
    ...
    if (!workingUtxos.length) {
      const errorMsg = window.getTranslation('transactions.no_suitable_utxos', `No suitable ${selectedAddressType} mature UTXOs available`);
      throw new Error(errorMsg);
    }
    ...
    if (isConsolidation) {
      ...
      if (total < target + finalFees) {
        ...
        const errorMsg = window.getTranslation('transactions.insufficient_funds_detailed', 
          `Insufficient funds. Use the MAX button or reduce the amount by ${shortfall.toFixed(8)} NITO to cover fees.`,
  ...
  async buildAndSignTransaction(to, target, selectedUtxos, isConsolidation, sourceAddressType) {
    ...
    let workingUtxos = selectedUtxos;
    if (!isConsolidation) {
      const filtered = filterUtxosByContext(selectedUtxos, false);
      workingUtxos = filtered.usableUtxos;
      
      if (workingUtxos.length === 0) {
        throw new Error('No sufficiently large UTXOs available for this transaction');
      }
    }
    ...
    const vbytes = this.feeManager.estimateVBytes(inputType, workingUtxos.length, outputCount);
    const fees = this.feeManager.calculateFeeForVsize(vbytes, realFeeRate);
    
    const total = workingUtxos.reduce((s, u) => s + Math.round(u.amount * 1e8), 0);
    const change = total - target - fees;
    
    if (change < 0) {
      const errorMsg = window.getTranslation('transactions.insufficient_funds_after_fees', 'Insufficient funds after fees');
      throw new Error(errorMsg);
    }
    ...
    for (const utxo of workingUtxos) {
    ...
    await this.signPsbtInputs(psbt, workingUtxos, sourceAddressType);
...
export async function consolidateUtxos() {
  ...
  try {
    const walletInfo = await getWalletInfo();
    if (!walletInfo.isReady) {
      const alertMsg = window.getTranslation('errors.import_first', 'Import a wallet first.');
      alert(alertMsg);
      return;
    }
    ...
    if (allUtxos.length < 2) {
      ...
      const alertMsg = window.getTranslation('transactions.need_at_least_utxos',
        `Need at least 2 mature UTXOs to consolidate. Found: ${allUtxos.length}`,
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        if (!window.rpc) {
          const errorMsg = window.getTranslation('errors.rpc_unavailable', 'RPC function unavailable');
          throw new Error(errorMsg);
        }
