...
async function getBitcoinLibraries() {
  ...
  if (!window.bitcoin || !window.ECPair) {
    const errorMsg = getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin libraries not properly loaded');
...
export class TaprootUtils {
  static toXOnly(pubkey) {
    if (!pubkey || pubkey.length < 33) {
      const errorMsg = getTranslation('wallet.invalid_pubkey_conversion', 'Invalid public key for X-only conversion');
      throw new Error(errorMsg);
    }
  ...
  static tapTweakHash(pubKey, h = Buffer.alloc(0)) {
    if (!window.bitcoin || !window.bitcoin.crypto) {
      const errorMsg = getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin library not available for tapTweakHash');
      throw new Error(errorMsg);
    }
  ...
  static tweakSigner(signer, opts = {}) {
    if (!window.ecc || typeof window.ecc.privateAdd !== 'function') {
      const errorMsg = getTranslation('wallet.ecc_library_unavailable', 'ECC library not available for tweakSigner');
      throw new Error(errorMsg);
    }
    ...
    if (!dTweak) {
      const errorMsg = getTranslation('wallet.invalid_tweaked_key', 'Invalid tweaked private key');
      throw new Error(errorMsg);
    }

    const PTweak = window.ecc.pointFromScalar(dTweak, true);
    if (!PTweak) {
      const errorMsg = getTranslation('transactions.failed_compute_tweaked_pubkey', 'Failed to compute tweaked public key');
      throw new Error(errorMsg);
    }
  ...
  static async createTaprootAddress(publicKey, network) {
    if (!window.bitcoin || !window.bitcoin.payments) {
      const errorMsg = getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin library not available');
      throw new Error(errorMsg);
    }
  ...
  static async prepareTaprootUtxo(utxo) {
    try {
      ...
      if (!kp || !kp.publicKey) {
        const errorMsg = getTranslation('transactions.missing_taproot_keypair', 'Missing taproot keypair for UTXO preparation');
        throw new Error(errorMsg);
      }
...
// === CONSOLIDATION POPUP ===
async function showConsolidationConfirmPopup(utxoCount, batches) {
