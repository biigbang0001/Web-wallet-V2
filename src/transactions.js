    ...
    return { 
      ...
      waitForConfirmation: async (txid) => {
        if (txid) {
          ...
          return await waitForTransactionConfirmation(txid);
        }
        return false;
      }
    };
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        if (i === batches.length - 1) {
          ...
          if (txids.length === 1) {
            console.log(`[CONSOLIDATION] Waiting for confirmation...`);
            setTimeout(async () => {
              const confirmed = await waitForTransactionConfirmation(txid);
              if (confirmed && window.refreshAllBalances) {
                console.log(`[CONSOLIDATION] Confirmed, refreshing balances...`);
