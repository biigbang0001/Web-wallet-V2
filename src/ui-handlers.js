...
export function copyToClipboard(elementId) {
  try {
    ...
    if (!element) {
      const errorMsg = getTranslation('security.element_not_found', 'Element not found');
      throw new Error(errorMsg);
    }

    if (element.classList.contains('blurred')) {
      const errorMsg = getTranslation('security.please_reveal_first', 'Please reveal content first');
      throw new Error(errorMsg);
    }

    const text = element.textContent || element.innerText || '';
    if (!text.trim()) {
      const errorMsg = getTranslation('security.nothing_to_copy', 'Nothing to copy');
      throw new Error(errorMsg);
    }
...
function fallbackCopy(text) {
  ...
  try {
    ...
    const errorMsg = getTranslation('security.copy_failed', 'Copy failed');
...
export function createSecureSeedButton(mnemonic, containerId) {
  ...
  seedButton.textContent = getTranslation('seed_reveal.button_reveal', 'Reveal mnemonic phrase');
  ...
  seedButton.addEventListener('click', () => {
    ...
    if (!isRevealed) {
      ...
      const warningText = getTranslation('seed_reveal.warning_title', 'Mnemonic phrase (24 words):');
      const copyButtonText = getTranslation('seed_reveal.copy_button', 'Copy');
      const timeoutWarning = getTranslation('seed_reveal.timeout_warning', 'This phrase will automatically be hidden in 30 seconds');
      ...
      document.getElementById('copySeedBtn').addEventListener('click', () => {
        ...
        if (navigator.clipboard && window.isSecureContext) {
          navigator.clipboard.writeText(mnemonic).then(() => {
            const successMsg = getTranslation('seed_reveal.copy_success', 'Mnemonic phrase copied to clipboard!');
            alert(successMsg);
          }).catch(() => {
            const textArea = document.createElement('textarea');
            ...
            document.body.removeChild(textArea);
            const fallbackMsg = getTranslation('seed_reveal.copy_fallback', 'Mnemonic phrase copied!');
            alert(fallbackMsg);
          });
        }
      });

      seedButton.textContent = getTranslation('seed_reveal.button_hide', 'Hide phrase');
      isRevealed = true;
