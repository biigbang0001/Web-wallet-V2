...
function createSecureSeedButton(mnemonic, containerId) {
  ...
  seedButton.textContent = t('seed_reveal.button_reveal', 'RÃ©vÃ©ler la phrase mnÃ©motechnique');
  ...
  seedButton.addEventListener('click', () => {
    ...
    if (!isRevealed) {
      ...
      const warningText = t('seed_reveal.warning_title', 'Phrase mnÃ©motechnique (24 mots) :');
      const copyButtonText = t('seed_reveal.copy_button', 'Copier');
      const timeoutWarning = t('seed_reveal.timeout_warning', 'Cette phrase sera automatiquement masquÃ©e dans 30 secondes');
      ...
      document.getElementById('copySeedBtn').addEventListener('click', () => {
        ...
        if (navigator.clipboard && window.isSecureContext) {
          navigator.clipboard.writeText(mnemonic).then(() => {
            const successMsg = t('seed_reveal.copy_success', 'Phrase mnÃ©motechnique copiÃ©e dans le presse-papiers !');
            alert(successMsg);
          }).catch(() => {
            ...
            const fallbackMsg = t('seed_reveal.copy_fallback', 'Phrase mnÃ©motechnique copiÃ©e !');
            alert(fallbackMsg);
          });
        }
      });
...
function setupGenerationHandlers() {
  addUniqueEventListener(ELEMENT_IDS.GENERATE_BUTTON, 'click', async () => {
    ...
    try {
      ...
      const errorMsg = t('errors.generation_failed', `Erreur de gÃ©nÃ©ration: ${error.message}`);
      alert(errorMsg);
...
function setupImportHandlers() {
  addUniqueEventListener(ELEMENT_IDS.IMPORT_WALLET_BUTTON, 'click', async () => {
    ...
    try {
      ...
      if (!input) {
        const errorMsg = t('errors.enter_key', 'Veuillez entrer une clÃ© privÃ©e, mnemonic ou XPRV');
        alert(errorMsg);
        return;
      }
      ...
      if (result.success) {
        ...
        const errorMsg = t('errors.import_failed', `Ã‰chec de l'import: ${result.error}`);
        alert(errorMsg);
        if (window.showBalanceLoadingSpinner) window.showBalanceLoadingSpinner(false);
      }
    } catch (error) {
  ...
  addUniqueEventListener(ELEMENT_IDS.CONNECT_EMAIL_BUTTON, 'click', async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorMsg = t('errors.connection_failed', `Ã‰chec de la connexion: ${result.error}`);
        alert(errorMsg);
        if (window.showBalanceLoadingSpinner) window.showBalanceLoadingSpinner(false);
      }
    } catch (error) {
...
function setupTransactionHandlers() {
  ...
  addUniqueEventListener(ELEMENT_IDS.MAX_BUTTON, 'click', async () => {
    ...
    if (maxButton) {
      ...
      maxButton.textContent = t('loading.calculating', 'Calcul...');
    }
    ...
    try {
      ...
      if (!window.calculateMaxSendableAmount) {
        throw new Error('Fonction calculateMaxSendableAmount non disponible');
      }
      ...
      const errorMsg = t('errors.max_calculation_failed', `Erreur de calcul MAX: ${error.message}`);
      alert(errorMsg);
    } finally {
      if (maxButton) {
  ...
  addUniqueEventListener(ELEMENT_IDS.PREPARE_TX_BUTTON, 'click', async () => {
    ...
    try {
      ...
      if (!to) {
        throw new Error(t('errors.enter_destination', 'Veuillez entrer une adresse de destination'));
      }
      
      if (!amount || amount <= 0) {
        throw new Error(t('errors.invalid_amount', 'Montant invalide'));
      }
      
      if (!window.signTxWithPSBT) {
        throw new Error(t('errors.transaction_functions_unavailable', 'Fonctions de transaction non disponibles'));
      }
      ...
      if (result.waitForConfirmation) {
        document.getElementById(ELEMENT_IDS.BROADCAST_TX_BUTTON).dataset.waitForConfirmation = 'true';
        document.getElementById(ELEMENT_IDS.BROADCAST_TX_BUTTON).dataset.resultObject = JSON.stringify(result);
      }
      ...
      const errorMsg = t('errors.transaction_prep_failed', `Ã‰chec de la prÃ©paration: ${error.message}`);
      alert(errorMsg);
  ...
  addUniqueEventListener(ELEMENT_IDS.BROADCAST_TX_BUTTON, 'click', async () => {
    ...
    try {
      ...
      if (!hex) {
        const errorMsg = t('errors.no_transaction', 'Aucune transaction Ã  diffuser');
        alert(errorMsg);
        return;
      }
      ...
      const shouldWaitForConfirmation = document.getElementById(ELEMENT_IDS.BROADCAST_TX_BUTTON).dataset.waitForConfirmation === 'true';
      const resultObjectStr = document.getElementById(ELEMENT_IDS.BROADCAST_TX_BUTTON).dataset.resultObject;
      
      if (shouldWaitForConfirmation && resultObjectStr) {
        try {
          const resultObject = JSON.parse(resultObjectStr);
          if (resultObject.waitForConfirmation) {
            console.log('[TX] Waiting for confirmation before refreshing balance...');
            setTimeout(async () => {
              const confirmed = await resultObject.waitForConfirmation(txid);
              if (confirmed && window.refreshAllBalances) {
                console.log('[TX] Confirmed, refreshing balances...');
                await window.refreshAllBalances();
              } else {
                console.log('[TX] No confirmation, doing delayed refresh...');
                setTimeout(() => {
                  if (window.refreshAllBalances) {
                    window.refreshAllBalances();
                  }
          ...
          console.error('[TX] Error with confirmation wait:', error);
          setTimeout(() => {
            if (window.refreshAllBalances) {
              window.refreshAllBalances();
            }
      ...
      const errorMsg = t('errors.broadcast_failed', `Ã‰chec de la diffusion: ${error.message}`);
      alert(errorMsg);
