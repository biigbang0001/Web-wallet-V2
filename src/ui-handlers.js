...
function createSecureSeedButton(mnemonic, containerId) {
  ...
  seedButton.addEventListener('click', () => {
    ...
    if (!isRevealed) {
      ...
      const warningText = t('seed_reveal.warning_title', 'Phrase mnémotechnique (24 mots) :');
...
function setupTransactionHandlers() {
  ...
  addUniqueEventListener(ELEMENT_IDS.PREPARE_TX_BUTTON, 'click', async () => {
    ...
    try {
      ...
      const errorMsg = t('errors.transaction_prep_failed', `Échec de la préparation: ${error.message}`);
      alert(errorMsg);
