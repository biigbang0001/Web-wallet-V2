...
async function getBitcoinLibraries() {
  ...
  if (!window.bitcoin || !window.ECPair || !window.bip39 || !window.bip32) {
    const msg = getTranslation('wallet.bitcoin_libraries_not_loaded', 'Bitcoin libraries not properly loaded');
    throw new Error(msg);
  }
...
export class TaprootUtils {
  static toXOnly(pubkey) {
    if (!pubkey || pubkey.length < 33) {
      const msg = getTranslation('wallet.invalid_pubkey_conversion', 'Invalid public key for X-only conversion');
      throw new Error(msg);
    }
  ...
  static tapTweakHash(pubKey, h = Buffer.alloc(0)) {
    if (!window.bitcoin || !window.bitcoin.crypto) {
      const msg = getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin library not available for tapTweakHash');
      throw new Error(msg);
    }
  ...
  static tweakSigner(signer, opts = {}) {
    if (!window.bitcoin || !window.bitcoin.crypto) {
      const msg = getTranslation('wallet.bitcoin_library_unavailable_tweaksigner', 'Bitcoin library not available for tweakSigner');
      throw new Error(msg);
    }
    ...
    if (!tweakedPrivateKey) {
      const msg = getTranslation('wallet.invalid_tweaked_key', 'Invalid tweaked private key');
      throw new Error(msg);
    }
  ...
  static async createTaprootAddress(publicKey, network) {
    try {
      if (!window.bitcoin || !window.bitcoin.payments) {
        const msg = getTranslation('wallet.bitcoin_library_unavailable_taproot', 'Bitcoin library not available');
        throw new Error(msg);
      }
...
export async function genAddr(type) {
  try {
    if (!['legacy', 'p2sh', 'bech32'].includes(type)) {
      const msg = getTranslation('wallet.invalid_address_type', 'Invalid address type');
      throw new Error(msg);
    }
