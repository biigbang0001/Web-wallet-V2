...
// === Bitcoin stack (CDN + versions pinned) ===
...
export function areLibrariesReady() { return !!librariesLoaded; }

export function waitForLibraries(timeout = 60000) {
  return new Promise((resolve, reject) => {
    if (areLibrariesReady()) { resolve(); return; }
...
// === Safe getters ===
export function getBitcoinLib() { if (!window.bitcoin?.payments) throw new Error('Bitcoin library not loaded'); return window.bitcoin; }
export function getECPair() { if (!window.ECPair?.makeRandom) throw new Error('ECPair not loaded'); return window.ECPair; }
export function getBip39() { if (!window.bip39?.generateMnemonic) throw new Error('BIP39 not loaded'); return window.bip39; }
export function getBip32() { if (!window.bip32?.fromSeed) throw new Error('BIP32 not loaded'); return window.bip32; }
export function getECC() { if (!window.ecc?.sign) throw new Error('ECC not loaded'); return window.ecc; }
export function getBuffer() { if (!window.Buffer?.from) throw new Error('Buffer not loaded'); return window.Buffer; }

export function resetLibrariesState() { librariesLoaded = false; loadingPromise = null; }
...
if (AUTO_LOAD && typeof window !== 'undefined') {
  setTimeout(() => { loadExternalLibraries().catch(err => console.error('Auto-loading failed:', err)); }, 100);
}

console.log('Vendor module loaded - Bitcoin libraries loading...');