  ...
  const executeAutoReload = () => {
    ...
    overlay.innerHTML = `
      ...
      ">
        ...
        <div style="font-size: 1.2rem; font-weight: 600; margin-bottom: 1rem;">Session expired</div>
        <div style="opacity: 0.8;">Reloading in progress...</div>
      </div>
    `;
...
export class NITOWalletApp {
  ...
  async initializeCore() {
    ...
    await this.initializeI18nCentralized();
  ...
  async initializeI18nCentralized() {
    return new Promise((resolve) => {
      try {
        ...
        const targetLng = localStorage.getItem('nito_lang') || 'en';
        ...
        window.i18next
          ...
          .init({
            ...
            this.setupLanguageSelector(changeLanguage, targetLng);
            resolve();
          });
      } catch (error) {
  ...
  markAsReady() {
    ...
    const readyMessage = getTranslation('system.wallet_ready', 
      `NITO Wallet ready in ${initTime}ms - Version ${VERSION.STRING}`, 
