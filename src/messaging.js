...
export class NitoMessaging {
  ...
  async sendMessage(message, recipientBech32Address) {
    ...
    try {
      ...
      this.updateProgressIndicator(0, 1, getTranslation('messaging.preparation_step', 'Preparation'));
  ...
  // === PROGRESS INDICATORS ===
  updateProgressIndicator(current, total, action = 'Sending') {
    ...
    if (progressElement) {
      ...
      progressElement.innerHTML = `
        <div style="text-align: center;">
          <div style="margin-bottom: 10px; font-weight: bold;">${analyzingText}</div>
...
window.testFullMessaging = async function() {
  try {
    ...
    console.log('[MESSAGING] Test successful:', result);

    await new Promise(resolve => setTimeout(resolve, 10000));

    const messages = await messaging.scanInboxMessages();

    console.log('[MESSAGING] Messages found:', messages.length);
...
console.log('Encrypted messaging module with Noble ECDH + AES-GCM loaded - Version 2.0.0');